...
export function getPreset(complete = true) {
    ...
    data[i++] = 0;    // We set device ID to 0 in order to get a sysex dump that can be sent to any Hedra.
    data[i++] = meta.group_id.value;
    data[i++] = meta.model_id.value;

    data[i++] = 0x26; // Hedra always sent this value when sending a sysex.
    ...
    data[i++] = control[control_id.scale].raw_value;
    ...
    data[i++] = control[control_id.scale].raw_value2;
...
export function getSysexDataForGlobalConfig(global_num, value) {
    ...
    // 03          Model #	(00 = Ottobit Jr, 01 = Mercury7, 02 = Hedra, 03 = Hedra)
